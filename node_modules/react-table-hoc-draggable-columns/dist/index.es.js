...
var index$1 = (function (Component) {
  var wrapper = function (_React$Component) {
    ...
    function RTFixedDraggableColumn(props) {
      ...
      _this.columnReorderedByHOC = false; // track if column reorder happened in or outside of ReactTable
      return _this;
    }
    ...
    createClass(RTFixedDraggableColumn, [{
        ...
        headers.forEach(function (header, i) {
          ...
          if (header.className.includes('enable-drag')) {
            ...
            if (enableColumnWideDrag) {
              headerParent.setAttribute(draggableAttr, true);
            } else {
              header.setAttribute(draggableAttr, true);
            ...
            headerParent.ondrop = function (e) {
              ...
              _this2.reorderIndicatorUp.style.display = 'none';
              _this2.reorderIndicatorDown.style.display = 'none';
              _this2.columnReorderedByHOC = true;
              // trigger a re-render
            ...
            var _headerParent = header.parentNode;

            var _props$draggableColum15 = _this2.props.draggableColumns.enableColumnWideDrag,
                _enableColumnWideDrag = _props$draggableColum15 === undefined ? defaultProps.enableColumnWideDrag : _props$draggableColum15;

            if (_enableColumnWideDrag) {
              _headerParent.setAttribute(draggableAttr, false);
            } else {
              header.setAttribute(draggableAttr, false);
            }
          }
        });
      }
      ...
      value: function componentDidUpdate(prevProps) {
        ...
        if (!this.state.firstLoad && this.columnReorderedByHOC) {
          ...
          this.columnReorderedByHOC = false;
        } else if (!this.state.firstLoad && !this.columnReorderedByHOC) {
          // the column order has been modified outside of this HOC
          // reset this.reorder array since it is no longer valid
          var _originalOrder = prevProps.columns.map(function (col) {
            if (typeof col.accessor === 'function') return col.id;
            return col.accessor;
          });

          var _newOrder = this.props.columns.map(function (col) {
            if (typeof col.accessor === 'function') return col.id;
            return col.accessor;
          });

          if (this.reorder.length > 0 && JSON.stringify(_originalOrder) !== JSON.stringify(_newOrder)) {
            this.reorder = [];
      ...
      value: function render() {
        ...
        var adjustedOrigColumns = [].concat(toConsumableArray(visibleColumns), toConsumableArray(hiddenColumns));
        ...
        var cols = adjustedOrigColumns.map(function (col, index) {
          ...
          return _extends({}, col, {
            Header: typeof col.Header === 'function' ? React.createElement(
              'div',
              { className: headerClassName, 'data-column-index': index },
              col.Header()
            ) : React.createElement(
              'div',
              { className: headerClassName, 'data-column-index': index },
        ...
        this.previousOrder = [].concat(toConsumableArray(this.currentColumnOrder));
